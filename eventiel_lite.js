...
- Markyjoe1990 and Nintenlord for the FE7 event template.
...
function setUp(eventFormDictionary, selectedMaps, fe7Defs, zhackDefs, FE7Chapters, FE7Music, FE7Backgrounds, FE7Spells)
{
	updateSelectOptions(".MapChoices", FE7Chapters[1]);
	updateSelectOptions("#sel_chosenChapter",FE7Chapters[0]);
	draw();
	selectDef(fe7Defs);
	updateEventOutput();
	setSelectedMainFormVisible();
}
 ...
 */
function setSelectedMainFormVisible()
{
	var formIdArray = ["topUnitForm","unitEditorWrapper","mapWrapper", "unitPointerImgDiv"];
	for(var i = 1; i < formIdArray.length; i++)
		document.getElementById(formIdArray[i]).style.display = "";
}
 ...
 */
function switchMainForm(form, eventFormDictionary, fe7Defs)
{
...
function switchToDefMaker(fe7Defs)
{
	updateDefType(fe7Defs);
...
function resetBlocks()
{
	...
	document.topUnitForm.txt_newUnitBlock.value = "Good";
	addBlock("Unit");
	document.topUnitForm.txt_newUnitBlock.value = "Bad";
	addBlock("Unit");
}
...
function draw() 
{
	...
	// Load the image
	img.src = document.topUnitForm.hiddenMap.value;
		...
		if(lockedBlock === null)
		{
			...
			if(idArray !== null)
			{
				...
				if(max_x > mapWidth || max_y > mapHeight)
				{
					...
					newImg.setAttribute('src',"https://dl.dropbox.com/u/71611130/BlockIcons/Locked.png");
...
function loadSelectedMap()
{
	...
	var selVal = getSelectedOption(document.topUnitForm.mapSelection);
	// Gets the value after the space in the dropdown list item
	var mapIndex = mapIdDict[selVal].split(" ");
	document.topUnitForm.hiddenMap.value = "https://dl.dropbox.com/u/71611130/ChapterMaps/" + mapIndex.pop() + ".png";
	draw();
	updateEventOutput();
}
...
var mapIdDict = {
	...
	"Ch6 0x06":"Chapter 6",
	"Ch7 0x07":"Chapter 7",
	"Ch7x 0x08":"Chapter 7x",
	"Ch8 0x09":"Chapter 8",
	"Ch9 0x0A":"Chapter 9",
	"Ch10 0x0B":"Chapter 10",
	"Ch11_E 0x0C":"Chapter 11",
	"Ch11_H 0x0D":"Chapter 11H",
	"Ch12 0x0E":"Chapter 12",
	"Ch13 0x0F":"Chapter 13",
	"Ch13x 0x10":"Chapter 13x",
	"Ch14 0x11":"Chapter 14",
	"Ch15 0x12":"Chapter 15H",
	"Ch16 0x13":"Chapter 15",
	"Ch17 0x14":"Chapter 16",
	"Ch17x 0x15":"Chapter 16x",
	"Ch18 0x16":"Chapter 17",
	"Ch19 0x17":"Chapter 18",
	"Ch19x 0x18":"Chapter 18x",
	"Ch19xpt2 0x19":"Chapter 19xx",
	"Ch20 0x1A":"Chapter 19",
	"Ch21 0x1B":"Chapter 20",
	"Ch22 0x1C":"Chapter 21",
	"Ch23 0x1D":"Chapter 22",
	"Ch23x 0x1E":"Chapter 22x",
	"Ch24_Lloyd 0x1F":"Chapter 23A",
	"Ch24_Linus 0x20":"Chapter 23B",
	"Ch25_H 0x21":"Chapter 25H",
	"Ch26 0x22":"Chapter 24",
	"Ch27_Kenneth 0x23":"Chapter 25A",
	"Ch27_Jerme 0x24":"Chapter 25B",
	"Ch28 0x25":"Chapter 26",
	"Ch28x 0x26":"Chapter 26x",
	"Ch29 0x27":"Chapter 27",
	"Ch28_E 0x28":"Chaper 28",
	"Ch30_H 0x29":"Chapter 30H",
	"Ch31 0x2A":"Chapter 29",
	"Ch31x 0x2B":"Chapter 29x",
	"Ch32 0x2C":"Chapter 30",
	"Ch32x_H 0x2D":"Chapter 32x",
	"Final_pt1 0x2E":"Final_Chapter_Pt1 31",
	"Final_pt2 0x2F":"Final_Chapter_Pt2 32"
}
...
function clickImg(unitNum, listNum)
{
	...
	setSelectedOption("Edit", document.topUnitForm.sel_mapClickMode);
	setMapClickMode();
...
function addUnit(mapSpriteDict)
{
	...
	if(!isBlockLocked(listNum))
	{
		...
		imgRightClick = "removeUnit(" + num + ", " + listNum + "); return false;";
		newImg.setAttribute('onclick', imgClick);
		newImg.setAttribute('oncontextmenu', imgRightClick);
...
function selectDef(fe7Defs)
{
	clearAllOptions();
	setDefinitionArray(fe7Defs[0],".Char");
	setDefinitionArray(fe7Defs[1],".Classes");
	setDefinitionArray(fe7Defs[2],".Item");
	document.unitEditorForm.btn_addUnit.disabled = "";
}
...
function updateDefType(fe7Defs)
{
	var choice = getSelectedOption(document.defMakerForm.sel_defType);
	if(choice == "Character")
		updateSelectOptions("#sel_origDef",fe7Defs[0]);
	else if(choice == "Class")
		updateSelectOptions("#sel_origDef",fe7Defs[1]);
	else
		updateSelectOptions("#sel_origDef",fe7Defs[2]);
}
...
{
	var eventTableId = getSelectedOption(document.topUnitForm.sel_chosenChapter).match(/0x[0-9A-Z][0-9A-Z]/);
	var goodUnitBlock = getEventCodeOfUnitBlock();
	var selVal = getSelectedOption(document.topUnitForm.mapSelection);
	var loadedMapId = selVal.split(" ").pop();
	var offset = (document.topUnitForm.removeEvents.checked)?("0x1000000"):("0xD80000")
	...
	"Opening_event:\nLOMA " + loadedMapId + " [7,5]\nHIDEMAP\nLOU1 Good\nENUN\nSHOWMAP\nOOBB\nENDA\n\n" +
	"Ending_event:\nTheEnd\nENDA\n\n" +
	"MESSAGE Events end at offset currentOffset";
}

function toggleRemoveEvents()
{
	if(document.topUnitForm.removeEvents.checked)
	{
		// Set selected option to match the map
		loadSelectedMap();
		document.topUnitForm.sel_chosenChapter.options[document.topUnitForm.mapSelection.selectedIndex].selected = "selected";
	}
	else
	{
		// Set selected option to be Prologue
		setSelectedOption("Prologue 0x06", document.topUnitForm.sel_chosenChapter);
	}
	updateEventOutput();
...
function getEventActionData()
{
	...
	if(actionType == "load_unit")
		return [retVal + multiBlock + ")", "LOU1 " + multiBlock.replace(/,/g," ") + "\n"];
	...
	if(actionType == "move_to_next_chapter")
		return [label,"MoveToChapter(" + chapterId + ")\n"];
...
function getEventCodeOfConditionBlocks()
{
	...
	retCode += "TurnEventPlayer(0x0,Opening_event,1)\nEnd_MAIN\n\n";
	...
	retCode += "DefeatBoss(Ending_event)\n";
	retCode += "DefeatAll(Ending_event)\nEnd_MAIN\n\n";
...
function getEventCodeOfUnitBlock()
{
	retCode = "";
	blockId = "b_name0";
	retCode += document.getElementById(blockId).innerHTML.split("<")[0] + "\n";
	var idArray = getAllIdsInBlock(0, 'Unit');
	if(idArray !== null && idArray !== undefined)
	{
		for(var k = 0; k < idArray.length; k++)
		{
...
function updateEventFormInfo(string)
{
	...
	if(partsArray[1] === null || partsArray[1] === undefined)
	{
		...
		// move_to_next_chapter(chaptId)
			...
			return "MoveToChapter(0x" + hexNum1 + ")\n";
...
function addManualMovement(type, num)
{
	...
	if(moveNum <= 60)
	{
		...
		else
		{
			...
			var imageUrl = "https://dl.dropbox.com/u/71611130/MoveArrows/" + color +"_" + direction + ".png";
...
var existingPointerNames = ["Good", "Bad", "Opening_event", "Ending_event",
						   "Turn_events", "Character_events",
						   "Location_events", "Misc_events", "TrapData",
