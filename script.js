...
{
	$('#topOutputForm').find('input[type=checkbox]:checked').removeAttr('checked');
	getAvailableEventIds();
	updateSelectOptions(".MapChoices",selectedMaps);
	updateSelectOptions("#sel_chosenChapter",FE7Chapters[0]);
	updateSelectOptions(".ChaptIdsList",FE7Chapters[1]);
	updateSelectOptions(".MusicList",FE7Music);
	updateSelectOptions(".BackgroundList",FE7Backgrounds);
	updateSelectOptions(".SpellList",FE7Spells);
	draw();
	selectDef(fe7Defs, zhackDefs);
	switchEventForm();
	updateEventOutput();
	switchMainForm("EventOutput", eventFormDictionary, fe7Defs);
...
function selectDef(fe7Defs, zhackDefs)
{
	clearAllOptions();
